...
def tampilkan_riwayat_pembelian():
    ...
    if result:
        print("{:<10} {:<15} {:<15} {:<15} {:<20} {:<15} {:<15}".format(
            "ID Booking", "Check In", "Check Out", "Cara Bayar", "Total Harga", "Nama", "No. Telp"
        ))
        print("=" * 100)
        for row in result:
...
def tampilkan_riwayat_pembelian_user(id_user, cursor):
    select_query = '''
    SELECT
        type `tipe_kamar`,
        id_user,
        ...
        total_pembelian
    FROM booking
    LEFT JOIN rooms ON booking.id_rooms = rooms.id_rooms
    WHERE id_user = %s
    '''
...
Tanggal check-in: {check_in.strftime('%A, %d %B %Y')}
Tanggal check-out: {check_out.strftime('%A, %d %B %Y')}
Atas nama: {nama} ({telp})
Pembayaran: {total_pembelian} via {cara_bayar}
---''')
    ...
    while True:
        ...
        while True:
            ...
            if validate_password(password):
                ...
                print("Invalid password. Passwordnya minimal 8 karakter.")
...
def select_rooms(cursor):
    ...
    if result:
        print("{:<10} {:<20} {:<10} {:<10}".format("ID", "Type", "Price", "Status"))
        print("=" * 50)
        for row in result:
            print("{:<10} {:<20} {:<10} {:<10}".format(row[0], row[1], row[2], row[3]))
    else:
        print("Tidak ada data kamar untuk ditampilkan.")
                        ...
                        while True:
                            ...
                            if role == 'admin':
                                if submenu_choice == '1':
                                    ...
                                    print("Pilihan tidak valid. Silakan pilih antara 4-11.")
                            else:
                                if submenu_choice == '1':
...
---''')
                    ...
                    if register_choice == '1':
                        ...
                        pass  # Do nothing, exit registration
                    else:
